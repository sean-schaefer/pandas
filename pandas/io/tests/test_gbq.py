...
class TestGBQConnectorIntegration(tm.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        with open('data/gcloud_credentials', 'r') as fin:
            creds_json = fin.read()
        ...
        cls.creds_file_valid.write(creds_json_valid)
        cls.creds_file_valid.flush()

        cls.creds_file_invalid = NamedTemporaryFile()
        cls.creds_file_invalid.write(creds_json_invalid)
        cls.creds_file_invalid.flush()

        cls.non_creds_file = NamedTemporaryFile()
        cls.non_creds_file.write('{"token": "50414e444153204556455259574845524521"}')
